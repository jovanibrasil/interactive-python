...
class Ship:
    ...
    def shoot(self):
        ...
        a_missile = Sprite(missile_pos, missile_vel, 1, 1, missile_image, missile_info, missile_sound)
...
def draw(canvas):
    ...
    canvas.draw_text("Lives", [25, 50], 30, "White")
    canvas.draw_text(str(lives), [25, 70], 20, "White")
    
    canvas.draw_text("Score", [WIDTH - 100, 50], 30, "White")
    canvas.draw_text(str(score), [WIDTH - 100, 70], 20, "White")
...
def rock_spawner():
    ...
    angle_vel = 1.0 / float(random.randint(10,15)) 
    vel = [float(random.randint(-3,3))/2, float(random.randint(-3,3))/2]
    a_rock = Sprite([x, y], vel, random.randint(0,5), angle_vel, asteroid_image, asteroid_info)
