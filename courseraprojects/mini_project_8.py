...
class Sprite:
    ...
    def update(self):
        ...
        if self.pos[0] > WIDTH - 10 or self.pos[1] > HEIGHT -10 or self.pos[0] < 0 or self.pos[1] < 0:
            return False
        # update position
        self.pos[0] = (self.pos[0] + self.vel[0]) % WIDTH
        self.pos[1] = (self.pos[1] + self.vel[1]) % HEIGHT
...
def draw(canvas):
    global time, started, score, lives, soundtrack
    ...
    b = group_collide(rock_group, my_ship)
...
def process_sprite_group(canvas):
    ...
    for a_missile in missile_group:
        ...
        if a_missile.update() is False:
            missile_group.remove(a_missile)
        
def group_collide(group, other_object):
    b = False
    rock_group = set(group)
    for a_rock in rock_group:
...
def rock_spawner():
    global rock_group
    if len(rock_group) < 13 and started is True:
